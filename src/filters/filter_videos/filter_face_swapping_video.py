...
from src.filters.filter_images.filter_face_swapping_image import FilterFaceSwappingImage

class FilterFaceSwappingVideo():
    def __init__(
        self,
        output_folder: str,
        filter_image: FilterFaceSwappingImage
    ):
        self.output_folder = output_folder
        self.filter_image = filter_image

    def filter_face_swapping_video(
        self,
        input_path: str = None,
        ...
        while(cap.isOpened()):
            ...
            if ret:
                ...
                for face in faces:
                    ...
                    keypoints = keypoints_array.astype(np.int32)

                    frame = self.filter_image.face_swapping(
                        image = frame,
                        filter = self.filter_image.filter,
                        keypoints = keypoints
                    )
                    ...
                    frame = draw_image_with_keypoints(
                        ...
                        keypoints= torch.from_numpy(next_keypoints),
                        width = frame.shape[1],
...
if __name__ == '__main__':
    ...
    @hydra.main(config_path = config_path, config_name = 'filter_face_swapping_video.yaml')
    def main(cfg: DictConfig):
        ...
        cfg.filter_image.csv_folder = csv_folder
        cfg.filter_image.model.pretrained_weight_path = pretrained_weight_path
        cfg.output_folder = output_folder

        filter_face_swapping_video = hydra.utils.instantiate(cfg)
        filter_face_swapping_video.filter_face_swapping_video(
            input_path = None,
            output_name = 'test_glasses_video4.mp4',
        )
